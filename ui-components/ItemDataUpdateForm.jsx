...
import { getItemData } from "./graphql/queries";
import { updateItemData } from "./graphql/mutations";
...
export default function ItemDataUpdateForm(props) {
  const {
    id: idProp,
    itemData: itemDataModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = itemDataRecord
      ? { ...initialValues, ...itemDataRecord }
      : initialValues;
    ...
    setNavigations(cleanValues.Navigations ?? []);
    setCurrentNavigationsValue("");
    setErrors({});
  };
  const [itemDataRecord, setItemDataRecord] = React.useState(itemDataModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getItemData.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getItemData
        : itemDataModelProp;
      setItemDataRecord(record);
    };
    queryData();
  }, [idProp, itemDataModelProp]);
  React.useEffect(resetStateValues, [itemDataRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateItemData.replaceAll("__typename", ""),
            variables: {
              input: {
                id: itemDataRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ItemDataUpdateForm")}
      {...rest}
    >
      <TextField
        label="Template"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || itemDataModelProp)}
